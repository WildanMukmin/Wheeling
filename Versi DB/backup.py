        ...
        if student_id.isdigit():
            ...
            if assessments:
                ...
                for assessment in assessments:
                    ...
                    assessments_by_date[date].append((assessment[1], assessment[2]))

                for date, assessments in assessments_by_date.items():
                    self.text_display_assessments.insert(ctk.END, f"Date: {date}\n")
                    for aspect, score in assessments:
                        self.text_display_assessments.insert(ctk.END, f"  {aspect.capitalize()}: {score}\n")
            else:
